class IsolateJob < ApplicationJob
  ...
  def initialize_workdir
    ...
    # Docker Desktop on macOS may not expose legacy cgroup paths isolate expects
    # for --cg. Fallback to non-cgroup mode so sandbox creation still succeeds.
    if workdir.to_s.empty? || !Dir.exist?(workdir)
      @cgroups = ""
      @workdir = `isolate -b #{box_id} --init`.chomp
    end
    raise "Failed to initialize isolate sandbox." if workdir.to_s.empty? || !Dir.exist?(workdir)
    @boxdir = workdir + "/box"
    @tmpdir = workdir + "/tmp"
  ...
  def extract_archive
    ...
    #{cg_timing_flag} \
    #{memory_limit_flag(Config::MAX_MEMORY_LIMIT)} \
  ...
  def run
    ...
    #{cg_timing_flag} \
    #{memory_limit_flag(submission.memory_limit)} \
  ...
  def cg_timing_flag
    ...
    submission.enable_per_process_and_thread_time_limit ? "--no-cg-timing" : "--cg-timing"
  end

  def memory_limit_flag(limit)
    cgroups.present? ? "--cg-mem=#{limit}" : "-m #{limit}"
  end
